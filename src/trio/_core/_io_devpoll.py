...
class _DevpollStatistics:
    tasks_waiting_read: int
    tasks_waiting_write: int
    backend: Literal["devpoll"] = attrs.field(init=False, default="devpoll")
...
class DevpollIOManager:
    _devpoll: select.devpoll = attrs.Factory(select.devpoll)
    # {fd: DevpollWaiters}
    _registered: defaultdict[int, DevpollWaiters] = attrs.Factory(
        lambda: defaultdict(DevpollWaiters)
    )
    _force_wakeup: WakeupSocketpair = attrs.Factory(WakeupSocketpair)
    _force_wakeup_fd: int | None = None

    def __attrs_post_init__(self) -> None:
        self._devpoll.register(self._force_wakeup.wakeup_sock, select.POLLIN)
        self._force_wakeup_fd = self._force_wakeup.wakeup_sock.fileno()

    def statistics(self) -> _DevpollStatistics:
    ...
    def get_events(self, timeout: float) -> EventResult:
        return self._devpoll.poll(timeout)
    ...
    def process_events(self, events: EventResult) -> None:
        for fd, flags in events:
            ...
            # with POLLHUP or POLLERR flags wakes both readers and
            # writers.
            if flags & ~select.POLLIN and waiters.write_task is not None:
                _core.reschedule(waiters.write_task)
                waiters.write_task = None
            if flags & ~select.POLLOUT and waiters.read_task is not None:
    ...
    def _update_registrations(self, fd: int) -> None:
        ...
        if waiters.read_task is not None:
            wanted_flags |= select.POLLIN
        if waiters.write_task is not None:
            wanted_flags |= select.POLLOUT
        if wanted_flags != waiters.current_flags:
            try:
                try:
                    # First try EPOLL_CTL_MOD
                    self._devpoll.modify(fd, wanted_flags)
                except OSError:
                    # If that fails, it might be a new fd
                    self._devpoll.register(fd, wanted_flags)
    ...
    async def _devpoll_wait(self, fd: int | _HasFileNo, attr_name: str) -> None:
    ...
    async def wait_readable(self, fd: int | _HasFileNo) -> None:
        ...
        """
        await self._devpoll_wait(fd, "read_task")
    ...
    async def wait_writable(self, fd: int | _HasFileNo) -> None:
        ...
        :raises trio.ClosedResourceError:
            ...
            function is still working.
        """
        await self._devpoll_wait(fd, "write_task")
