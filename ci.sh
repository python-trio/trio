...
if [ "$CHECK_FORMATTING" = "1" ]; then
    ...
    if COVERAGE_PROCESS_START=$(pwd)/../.coveragerc coverage run --rcfile=../.coveragerc -m pytest -ra --junitxml=../test-results.xml --run-slow ${INSTALLDIR} --verbose --durations=10 $flags; then
        PASSED=true
    else
        PASSED=false
    fi
