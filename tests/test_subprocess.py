...
from tests._core.tutil import skip_if_fbsd_pipes_broken, slow
from trio.lowlevel import open_process
from trio.testing import assert_no_checkpoints, wait_all_tasks_blocked
...
def test_waitid_eintr():
    ...
    from trio._subprocess_platform import wait_child_exiting

    if not wait_child_exiting.__module__.endswith("waitid"):
        pytest.skip("waitid only")

    from trio._subprocess_platform.waitid import sync_wait_reapable
