...
async def wait_child_exiting(process: "_subprocess.Process") -> None:
    ...
    raise NotImplementedError from (  # pragma: no cover
        wait_child_exiting._error  # type: ignore
    )
...
def create_pipe_to_child_stdin() -> Tuple[SendStream, int]:
    ...
    """
    raise NotImplementedError from (  # pragma: no cover
        create_pipe_to_child_stdin._error  # type: ignore
    )
...
try:
    if sys.platform.startswith("win"):
        ...
        def create_pipe_to_child_stdin() -> Tuple[  # noqa: F811
            SendStream, int
        ]:
        ...
        def create_pipe_from_child_output() -> Tuple[  # noqa: F811
            ReceiveStream, int
        ]:
        ...
        sys.platform.startswith("darwin")
        or sys.platform.startswith("freebsd")
        or (not TYPE_CHECKING and sys.name == "posix")
    ):
    ...
    create_pipe_to_child_stdin._error = ex  # type: ignore
    create_pipe_from_child_output._error = ex  # type: ignore
